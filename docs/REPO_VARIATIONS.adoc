...
|AffineScript |~18 |idaptiky, broad-spectrum, UbiCity, fogbinder
...
|Guix-only |~35 |STATE.scm, git-hud, wp-praxis
|===
...
==== Language Topics
- `rust`, `affinescript`, `elixir`, `zig`, `ada`, `haskell`, `racket`, `nickel`
...
==== AffineScript Projects (affinescript.json)
[source,json]
----
{
  "name": "project-name",       // VARIES
...
|`build` |`cargo build`, `affinescript build`, `mix compile`
...
|Contributing |COMMON |Reference to ../.github/CONTRIBUTING.md
|===
...
[cols="1,1,1,1,1,1"]
|===
|Aspect |Rust Binary |Rust Library |AffineScript |Elixir |Guix-only

|Entry point |src/main.rs |src/lib.rs |src/Main.res |lib/app.ex |flake.guix
|Build cmd |cargo build |cargo build |npm run build |mix compile |guix build
|Test cmd |cargo test |cargo test |npm test |mix test |guix check
|Package file |Cargo.toml |Cargo.toml |package.json |mix.exs |flake.guix
...
idaptiky,affinescript,1,pending,partial,"affinescript,web",Web app
...
----
