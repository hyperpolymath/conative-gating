...
[source,bash]
----
guix shell -D -f build/guix.scm  # Enter dev environment
...
=== Guix/Guix Infrastructure
[source]
----
+── guix.scm               # Guix package
+── .guix-channel          # Channel definition
+── flake.guix              # Guix flake (fallback)
+── flake.lock             # Guix lockfile
└── modules/               # Scheme modules
----
...
|`tsconfig.json`
|TypeScript deprecated (use AffineScript)
...
* Guix/Guix configuration exists
