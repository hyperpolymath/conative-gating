...
* **Tier 1**: Rust, Elixir, Zig, Ada/SPARK, Haskell, AffineScript
* **Tier 2**: Nickel, Racket, Guile Scheme, Guix
...
rsr_tier_1=(rust elixir zig ada haskell affinescript)
rsr_tier_2=(nickel racket guile guix)
...
# - build.zig → zig build
# - *.gpr → gprbuild
# - affinescript.json → affinescript

# Run tests by tier
rsr test --tier 1
----

== Proposal 4: Guix/Guix Package Management Primitives

=== Rationale

RSR uses Guix as primary package manager with Guix as fallback. Native primitives would:
...
==== Guix Primitives (Fallback)

[source,bash]
----
# Enter Guix shell
guix shell nixpkgs#rust nixpkgs#cargo
...
[source,bash]
----
# Auto-detect Guix vs Guix
pkg shell rust cargo
# Uses Guix if available, falls back to Guix
