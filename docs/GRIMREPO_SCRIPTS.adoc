...
* **AffineScript first** - Type-safe, compiles to clean JS
...
module AibdpChecker = {
  ...
  let displayBadge = (policy: option<aibdpPolicy>): unit => {
    ...
    | Some(p) if p.training == "refused" =>
        badge->setInnerHTML("🔒 AI Training Refused")
    | Some(p) if p.training == "conditional" =>
        badge->setInnerHTML("⚠️ AI Training Conditional")
    | Some(p) if p.training == "allowed" =>
        badge->setInnerHTML("✓ AI Training Allowed")
    | None =>
        badge->setInnerHTML("❓ No AIBDP")
    }
  }
}
----
...
==== 19. `grim-affinescript-highlighter.res`

**Purpose**: Enhanced AffineScript syntax highlighting
...
│           └── lib.rs
├── dist/
│   ├── grim-aibdp-checker.user.js
│   ├── grim-citation-extractor.user.js
│   └── ...
├── affinescript.json
├── deno.json
├── Justfile
...
# Validate
check: check-affinescript check-types
    @echo "✓ All checks passed"

check-affinescript:
    npx affinescript format -check src/**/*.res

check-types:
    npx affinescript typecheck
----
...
13. `grim-affinescript-highlighter.res`
