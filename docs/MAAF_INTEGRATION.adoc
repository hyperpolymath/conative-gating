...
* **zoterho-template** — Zotero 7+ extension template (AffineScript, Deno, Guile)
...
* **AffineScript**: UI with Deno runtime
...
**Repository**: https://gitlab.com/hyperpolymath/bunsenite (canonical;
the former `campaign-for-cooler-coding-and-programming/bunsenite` path is
a stale mirror — conative-gating vendors the canonical tree under
`vendor/bunsenite/`, see `vendor/bunsenite/VENDOR.adoc`)
...
[source]
----
Rust (nickel-lang-core) → Zig (C ABI) → Bindings (Deno, AffineScript, C)
----
...
. **Guix/Guix Integration**: Native package management primitives
