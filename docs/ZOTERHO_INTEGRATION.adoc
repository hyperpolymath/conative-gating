...
|Migrate from TypeScript to AffineScript via zoterho-template
...
**Target State**: AffineScript + Deno + Guile (via zoterho-template)
...
cd zotero-nsai
git checkout -b v2-affinescript

# Copy zoterho structure
cp -r ../zoterho-template/src/affinescript src/
...
// AffineScript version of handoff.ts
...
[ ] Implement AffineScript logic in src/
...
[source]
----
[ ] Create v2-affinescript branch
[ ] Copy zoterho-template structure
[ ] Migrate TypeScript types to AffineScript
[ ] Migrate validation logic to AffineScript
[ ] Replace CUE with Guile configs
[ ] Update Fogbinder handoff to AffineScript
[ ] Run: just test
